...
async def generate_relic() -> LorePiece:
    """
    ...
    try:
        # Name prompt
        name_prompt = (
            f"Invent a unique name for a mysterious post-apocalyptic relic or artifact that does NOT contain or match any of the following words or names: {blacklist_str}. "
        ...
        # Description prompt
        description_prompt = (
            f"Describe the appearance and notable features of the relic named '{name}'."
        ...
        # History prompt
        history_prompt = (
            f"Explain the history and significance of the relic '{name}'."
            " Respond only with plain text, no markdown."
            " No newlines; output a single paragraph."
            " Respond in exactly 2 sentences."
        )
